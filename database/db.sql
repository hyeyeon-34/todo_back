...
-- 데이터 조회
SELECT * FROM task WHERE userId = 'hyeyeon' ORDER BY created_at DESC
...
-- 데이터 변경
UPDATE task SET iscompleted = 'false' WHERE title = 'text2'
...
-- BEFORE UPDATE 트리거는 레코드가 업데이트 되기 직전에 updated_at 필드를 현재 시간으로 변경.
